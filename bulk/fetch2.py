...
class RequestParams():
    ...
    @property
    def expected_response_content_type(self) -> ContentType:
        accept = dict(self.headers).get('accept', '')
        for c in ContentType:
            if c in accept or f'.{c}' in self.url:
                return c
...
class Response():
    ...
    def content_type(self) -> ContentType:
        for c in ContentType:
            if c in self.headers['content-type']:
                return c
        raise NotImplementedError()
...
async def fetch(
    ...
    try:
        ...
        async with session.request(**params.asdict(), timeout=5, ssl=False) as raw_response:
